...
def create():
    try:
        return json_response(Folder.create(**request.json).to_dict())
    except Exception, e:
        return error_response(str(e))
...
def get(id):
    try:
        return json_response(Folder.find(id).to_dict())
...
def update(id):
    ...
    try:
        return json_response(Folder.find(id).update(**request.json).to_dict())
    except AttributeError, e:
        return bad_id_response(id)
    except Exception, e:
        return error_response(str(e))
...
def delete(id):
    try:
        return json_response(Folder.find(id).delete())
    except AttributeError, e:
        return bad_id_response(id)
    except Exception, e:
        return error_response(str(e))
